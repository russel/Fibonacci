...
public class Fibonacci {
  ...
  private static void validate(final BigInteger n) {
    if (n.compareTo(zero) < 0) { throw new IllegalArgumentException("index must be positive."); }
  }
  ...
  public static BigInteger naïveRecursive(final  BigInteger n) {
    ...
    return
      ...
      n.compareTo(one) == 0 ? one : naïveRecursive(n.subtract(one)).add(naïveRecursive(n.subtract(two)));
  }

  static Map<BigInteger,BigInteger> memo = new HashMap<BigInteger,BigInteger>();
  static {
    memo.put(zero, zero);
    memo.put(one, one);
  }

  public static BigInteger memoizedRecursive(final long n) { return memoizedRecursive(BigInteger.valueOf(n)); }
  ...
  public static BigInteger closedForm(final long n) { return closedForm(BigInteger.valueOf(n)); }
  public static BigInteger closedForm(final BigInteger n) {
    validate(n);
    final BigDecimal sqrt5 = BigDecimal.valueOf(sqrt(5));
    return sqrt5
        .add(bd_one)
        .divide(bd_two)
        .pow(n.intValue())
  ...
  public static class Sequence {
    ...
    public static class Iterator implements java.util.Iterator<BigInteger> {
      ...
      @Override public BigInteger next() { return Sequence.getAt(currentIndex++); }
